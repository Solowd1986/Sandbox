...
class Overlay extends Component {
    ...
    destroy = () => {
        ...
        if (this.props.disableOverlayCallback) {
            this.props.disableOverlayCallback();
        }
    };
    ...
    /**
     * Ход работы:
     * 1. Данный компонент имеет два метода: disableOverlay/enableOberlay, они меняют boolean-свойство showModal
     * 2. Любой компонент желающий использовать модальное окно может получть метод enableOberlay, для включения модалльного окна
     * 3. При этом, изначальный показ/скрытие окна в компоненте опирается на boolean-свойство showModal, его нужно проверять
     * 4. Выключение модального окна по-умолчанию выполняется при клике на overlay и доступно по-умолчанию
     * 5. Любое нужное модальное содержимое просто оборачивается в данный компонент.
     * 6. Если нужна эмуляция запроса к серверу, то можно передать атрибут delay, чтобы сэмулировать задержку
     * 7. По истечении таймера boolean-свойство delayModal изменится и вращающйся спиннер исчезнет, замещенный желаемым модальным окном
    ...
    render() {
        ...
        return (
            <div onClick={this.destroy} className={`${styles.overlay} ${this.coloredBg && styles.overlay_bg}`}>
                <div>
                    {
                        this.props.delay && !this.props.isdelayModalProcessCompleted
                            ?
                            <svg
...
function mapDispatchToProps(dispatch) {
    return {
        disableOverlay: () => {
            ...
            dispatch(actions.lazyload.disableOverlay());
        },
        delayOrder: () => {
