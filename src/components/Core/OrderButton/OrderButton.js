...
const OrderButton = props => {
    return (
        <button
            ...
            /*
            * Классы: базово применится общий btn, далее - класс кнопки заказа. Потом класс серого фона кнопки, он визуально
            * общия для ситуаций отсуствия товара вообще или наличия товара в корзине. Последними - любые доп. классы извне.
            * */
            className={
                `
                ${common.btn} 
            ...
            // данный атрибут используется для передачи имени класса кнопки (для стилизации), для смены вида на активную/неактивную
            ...
            // 2. Из props получен state и в нем передана команда отлючения кнопки
            // 3. Из props получено свойство disabled равное true, это можно передать для любой выбранной кнопки
            disabled={props.product.rest === 0 || props.state.cart.defaultSettings.buttonsDisabled || props.disabled}>

            {/*Если продукт есть в наличии или текст кнопки не содержит текст "Убрать из заказа" - то показываем svg-картинку корзины*/}
            {/*Это потому, что данное изображение не нужно как в случае исключения товара из заказа, так и в случае его отсутствия в целом*/}
            {
            ...
            {/*Спиннер, появится при ожидании ответа от сервера*/}
