...
{
    $_SESSION["pwds"][$cj][$M][$V] = ($_COOKIE["adminer_key"] && is_string($E) ? [encrypt_string($E, $_COOKIE["adminer_key"])] : $E);
}
...
input($o, $Y, $r)
{
    ...
    $od = (isset($_GET["select"]) || $Qg ? ["orig" => lang(8)] : []) + $b->editFunctions($o);
    $Ka = " name='fields[$B]'";
    if ($o["type"] == "enum") echo
...
fields_from_edit()
{
    ...
    foreach ((array)$_POST["fields"] as $y => $X) {
        ...
        $H[$B] = ["field" => $B, "privileges" => ["insert" => 1, "update" => 1], "null" => 1, "auto_increment" => ($y == $m->primary),];
    }
    return $H;
}
...
search_tables()
{
    ...
    foreach (table_status('', true) as $Q => $R) {
        ...
        if (isset($R["Engine"]) && $B != "" && (!$_POST["tables"] || in_array($Q, $_POST["tables"]))) {
            $G = $h->query("SELECT" . limit("1 FROM " . table($Q), " WHERE " . implode(" AND ", $b->selectSearchProcess(fields($Q), [])), 1));
            if (!$G || $G->fetch_row()) {
...
edit_form($a, $p, $I, $Oi)
{
    ...
    page_header(($Oi ? lang(10) : lang(11)), $n, ["select" => [$a, $Uh]], $Uh);
...
if ($_GET["script"] == "version") {
    ...
    if ($md) file_write_unlock($md, serialize(["signature" => $_POST["signature"], "version" => $_POST["version"]]));
    exit;
}
...
Min_SQL
{
    ...
    function
    select($Q, $K, $Z, $rd, $Ef = [], $z = 1, $D = 0, $rg = false)
    {
        global $b, $x;
        $ce = (count($rd) < count($K));
...
if (isset($_GET["sqlite"]) || isset($_GET["sqlite2"])) {
    $mg = [(isset($_GET["sqlite"]) ? "SQLite3" : "SQLite"), "PDO_SQLite"];
        ...
        if (isset($_GET["sqlite"])) {
            ...
            Min_Result
            {
                ...
                fetch_field()
                {
                    ...
                    return (object)["name" => $B, "orgname" => $B, "orgtable" => $Q,];
                }
            }
        }
    ...
    fields($Q)
    {
        ...
        foreach (get_rows("PRAGMA table_info(" . table($Q) . ")") as $I) {
            ...
            $T = strtolower($I["type"]);
            $Wb = $I["dflt_value"];
            $H[$B] = ["field" => $B, "type" => (preg_match('~int~i', $T) ? "integer" : (preg_match('~char|clob|text~i', $T) ? "text" : (preg_match('~blob~i', $T) ? "blob" : (preg_match('~real|floa|doub~i', $T) ? "real" : "numeric")))), "full_type" => $T, "default" => (preg_match("~'(.*)'~", $Wb, $A) ? str_replace("''", "'", $A[1]) : ($Wb == "NULL" ? null : $Wb)), "null" => !$I["notnull"], "privileges" => ["select" => 1, "insert" => 1, "update" => 1], "primary" => $I["pk"],];
            if ($I["pk"]) {
    ...
    indexes($Q, $i = null)
    {
        ...
        if (preg_match('~\bPRIMARY\s+KEY\s*\((([^)"]+|"[^"]*"|`[^`]*`)++)~i', $Dh, $A)) {
            $H[""] = ["type" => "PRIMARY", "columns" => [], "lengths" => [], "descs" => []];
        ...
        if (!$H) {
            foreach (fields($Q) as $B => $o) {
                if ($o["primary"]) $H[""] = ["type" => "PRIMARY", "columns" => [$B], "lengths" => [], "descs" => [null]];
            }
        }
    ...
    function
    recreate_table($Q, $B, $p, $Nf, $gd, $Na, $w = [])
    {
        global $h;
        if ($Q != "") {
            if (!$p) {
            ...
            foreach (indexes($Q) as $ke => $v) {
                ...
                if (!$lc[$ke]) {
                    if ($v["type"] != "PRIMARY" || !$qg) $w[] = [$v["type"], $ke, $f];
                }
            }
            foreach ($w
                     as $y => $X) {
    ...
    trigger($B)
    {
        ...
        $of = $A[3];
        return
            ["Timing" => strtoupper($A[1]), "Event" => strtoupper($A[2]) . ($of ? " OF" : ""), "Of" => ($of[0] == '`' || $of[0] == '"' ? idf_unescape($of) : $of), "Trigger" => $B, "Statement" => $A[4],];
    }
    ...
    $Mh = array_keys($U);
    $Ni = [];
    $_f = ["=", "<", ">", "<=", ">=", "!=", "LIKE", "LIKE %%", "IN", "IS NULL", "NOT LIKE", "NOT IN", "IS NOT NULL", "SQL"];
    $od = ["hex", "length", "lower", "round", "unixepoch", "upper"];
    $ud = ["avg", "count", "count distinct", "group_concat", "max", "min", "sum"];
    $qc = [[], ["integer|real|numeric" => "+/-", "text" => "||",]];
}
$ic["pgsql"] = "PostgreSQL";
if (isset($_GET["pgsql"])) {
    $mg = ["PgSQL", "PDO_PgSQL"];
        ...
        Min_SQL
    {
        ...
        tableHelp($B)
        {
            $Ae = ["information_schema" => "infoschema", "pg_catalog" => "catalog",];
            $_ = $Ae[$_GET["ns"]];
...
ORDER BY conkey, conname") as $I) {
    ...
    {
        $H = [];
        foreach (get_rows("SELECT * FROM information_schema.triggers WHERE event_object_table = " . q($Q)) as $I) $H[$I["trigger_name"]] = [$I["action_timing"], $I["event_manipulation"]];
        return $H;
    }
...
AND typelem = 0");
    ...
    $Ni = [];
    $_f = ["=", "<", ">", "<=", ">=", "!=", "~", "!~", "LIKE", "LIKE %%", "ILIKE", "ILIKE %%", "IN", "IS NULL", "NOT LIKE", "NOT IN", "IS NOT NULL"];
    $od = ["char_length", "lower", "round", "to_hex", "to_timestamp", "upper"];
    $ud = ["avg", "count", "count distinct", "max", "min", "sum"];
    $qc = [["char" => "md5", "date|time" => "now",], [number_type() => "+/-", "date|time" => "+ interval/- interval", "char|text" => "||",]];
}
...
ORDER BY 1") as $I) {
    ...
    fields($Q)
    {
        ...
        foreach (get_rows("SELECT * FROM all_tab_columns WHERE table_name = " . q($Q) . " ORDER BY column_id") as $I) {
            ...
            $H[$I["COLUMN_NAME"]] = ["field" => $I["COLUMN_NAME"], "full_type" => $T . ($ye ? "($ye)" : ""), "type" => strtolower($T), "length" => $ye, "default" => $I["DATA_DEFAULT"], "null" => ($I["NULLABLE"] == "Y"), "privileges" => ["insert" => 1, "select" => 1, "update" => 1],];
        }
        return $H;
    }
...
AND c_src.TABLE_NAME = " . q($Q);
        foreach (get_rows($F) as $I) $H[$I['NAME']] = ["db" => $I['DEST_DB'], "table" => $I['DEST_TABLE'], "source" => [$I['SRC_COLUMN']], "target" => [$I['DEST_COLUMN']], "on_delete" => $I['ON_DELETE'], "on_update" => null,];
        return $H;
    }
...
');
    ...
    $Ni = [];
    $_f = ["=", "<", ">", "<=", ">=", "!=", "LIKE", "LIKE %%", "IN", "IS NULL", "NOT LIKE", "NOT REGEXP", "NOT IN", "IS NOT NULL", "SQL"];
    $od = ["length", "lower", "round", "upper"];
    $ud = ["avg", "count", "count distinct", "max", "min", "sum"];
    $qc = [["date" => "current_date", "timestamp" => "current_timestamp",], ["number|float|double" => "+/-", "date|timestamp" => "+ interval/- interval", "char|clob" => "||",]];
}
$ic["mssql"] = "MS SQL (beta)";
if (isset($_GET["mssql"])) {
    $mg = ["SQLSRV", "MSSQL", "PDO_DBLIB"];
    define("DRIVER", "mssql");
    if (extension_loaded("sqlsrv")) {
        ...
        Min_DB
        {
            ...
            connect($M, $V, $E)
            {
                global $b;
                $l = $b->database();
                $_b = ["UID" => $V, "PWD" => $E, "CharacterSet" => "UTF-8"];
                if ($l != "") $_b["Database"] = $l;
...
WHERE o.schema_id = SCHEMA_ID(" . q(get_schema()) . ") AND o.type IN ('S', 'U', 'V') AND o.name = " . q($Q)) as $I) {
            ...
            $H[$I["name"]] = ["field" => $I["name"], "full_type" => $T . ($ye ? "($ye)" : ""), "type" => $T, "length" => $ye, "default" => $I["default"], "null" => $I["is_nullable"], "auto_increment" => $I["is_identity"], "collation" => $I["collation_name"], "privileges" => ["insert" => 1, "select" => 1, "update" => 1], "primary" => $I["is_identity"], "comment" => $xb[$I["name"]],];
        }
        return $H;
    }
...
WHERE sys1.xtype = 'TR' AND sys2.name = " . q($Q)) as $I) $H[$I["name"]] = [$I["Timing"], $I["Event"]];
        return $H;
    }
    ...
    $Ni = [];
    $_f = ["=", "<", ">", "<=", ">=", "!=", "LIKE", "LIKE %%", "IN", "IS NULL", "NOT LIKE", "NOT IN", "IS NOT NULL"];
    $od = ["len", "lower", "round", "upper"];
    $ud = ["avg", "count", "count distinct", "max", "min", "sum"];
    $qc = [["date|time" => "getdate",], ["int|decimal|real|float|money|datetime" => "+/-", "char|text" => "+",]];
}
...
ORDER BY r.RDB$FIELD_POSITION';
        $G = ibase_query($h->_link, $F);
        while ($I = ibase_fetch_assoc($G)) $H[trim($I['FIELD_NAME'])] = ["field" => trim($I["FIELD_NAME"]), "full_type" => trim($I["FIELD_TYPE"]), "type" => trim($I["FIELD_SUB_TYPE"]), "default" => trim($I['FIELD_DEFAULT_VALUE']), "null" => (trim($I["FIELD_NOT_NULL_CONSTRAINT"]) == "YES"), "auto_increment" => '0', "collation" => trim($I["FIELD_COLLATION"]), "privileges" => ["insert" => 1, "select" => 1, "update" => 1], "comment" => trim($I["FIELD_DESCRIPTION"]),];
        return $H;
    }
...
if (isset($_GET["simpledb"])) {
    ...
    if (class_exists('SimpleXMLElement') && ini_bool('allow_url_fopen')) {
        ...
        Min_DB
        {
            ...
            query($F, $Hi = false)
            {
                $Tf = ['SelectExpression' => $F, 'ConsistentRead' => 'true'];
                ...
                if (preg_match('~^\s*SELECT\s+COUNT\(~i', $F)) {
                    ...
                    $G = [(object)['Attribute' => [(object)['Name' => 'Count', 'Value' => $Qh,]]]];
                }
                return
                    new
                    Min_Result($G);
        ...
        Min_Result
        {
            ...
            fetch_field()
            {
                ...
                return (object)['name' => $le[$this->_offset++]];
            }
        }
    }
        ...
        Min_SQL
    {
        ...
        _extractIds($Q, $Ag, $z)
        {
            ...
            if (preg_match_all("~itemName\(\) = (('[^']*+')+)~", $Ag, $Ie)) $H = array_map('idf_unescape', $Ie[1]); else {
                foreach (sdb_request_all('Select', 'Item', ['SelectExpression' => 'SELECT itemName() FROM ' . table($Q) . $Ag . ($z ? " LIMIT 1" : "")]) as $fe) $H[] = $fe->Name;
            }
            return $H;
        }
        ...
        delete($Q, $Ag, $z = 0)
        {
            return $this->_chunkRequest($this->_extractIds($Q, $Ag, $z), 'BatchDeleteAttributes', ['DomainName' => $Q]);
        }
        ...
        update($Q, $N, $Ag, $z = 0, $L = "\n")
        {
            ...
            foreach ($N
                     as $y => $X) {
                $y = idf_unescape($y);
                if ($X == "NULL" || ($t != "" && [$t] != $Id)) $Zb["Attribute." . count($Zb) . ".Name"] = $y;
                if ($X != "NULL") {
                    foreach ((array)$X
                             as $he => $W) {
            ...
            $Tf = ['DomainName' => $Q];
            return (!$Xd || $this->_chunkRequest(($t != "" ? [$t] : $Id), 'BatchPutAttributes', $Tf, $Xd)) && (!$Zb || $this->_chunkRequest($Id, 'BatchDeleteAttributes', $Tf, $Zb));
        }
    ...
    table_status($B = "", $Sc = false)
    {
        ...
        foreach (($B != "" ? [$B => true] : tables_list()) as $Q => $T) {
            ...
            if (!$Sc) {
                $Ve = sdb_request('DomainMetadata', ['DomainName' => $Q]);
                if ($Ve) {
                    foreach (["Rows" => "ItemCount", "Data_length" => "ItemNamesSizeBytes", "Index_length" => "AttributeValuesSizeBytes", "Data_free" => "AttributeNamesSizeBytes",] as $y => $X) $I[$y] = (string)$Ve->$X;
                }
            }
            if ($B != "") return $I;
            $H[$Q] = $I;
    ...
    drop_tables($S)
    {
        foreach ($S
                 as $Q) {
            if (!sdb_request('DeleteDomain', ['DomainName' => $Q])) return
                false;
        }
        return
            true;
    ...
    sdb_request($wa, $Tf = [])
    {
        ...
        $Xc = @file_get_contents((preg_match('~^https?://~', $Ed) ? $Ed : "http://$Ed"), false, stream_context_create(['http' => ['method' => 'POST', 'content' => $F, 'ignore_errors' => 1,]]));
        if (!$Xc) {
    ...
    $x = "simpledb";
    $_f = ["=", "<", ">", "<=", ">=", "!=", "LIKE", "LIKE %%", "IN", "IS NULL", "NOT LIKE", "IS NOT NULL"];
    $od = [];
    $ud = ["count"];
    $qc = [["json"]];
}
...
if (isset($_GET["mongo"])) {
    ...
    if (class_exists('MongoDB')) {
            ...
            Min_SQL
        {
            ...
            select($Q, $K, $Z, $rd, $Ef = [], $z = 1, $D = 0, $rg = false)
            {
                ...
                return
                    new
                    Min_Result($this->_conn->_db->selectCollection($Q)->find([], $K)->sort($_h)->limit($z != "" ? +$z : 0)->skip($D * $z));
            }
        ...
        indexes($Q, $i = null)
        {
            ...
            foreach ($h->_db->selectCollection($Q)->getIndexInfo() as $v) {
                ...
                $H[$v["name"]] = ["type" => ($v["name"] == "_id_" ? "PRIMARY" : ($v["unique"] ? "UNIQUE" : "INDEX")), "columns" => array_keys($v["key"]), "lengths" => [], "descs" => $cc,];
            }
            return $H;
        }
            ...
            Min_SQL
        {
            ...
            select($Q, $K, $Z, $rd, $Ef = [], $z = 1, $D = 0, $rg = false)
            {
                ...
                $F = new$lb($Z, ['projection' => $K, 'limit' => $z, 'skip' => $xh, 'sort' => $_h]);
        ...
        indexes($Q, $i = null)
        {
            ...
            foreach ($Ug
                ...
                $H[$v->name] = ["type" => ($v->name == "_id_" ? "PRIMARY" : (isset($v->unique) ? "UNIQUE" : "INDEX")), "columns" => $f, "lengths" => [], "descs" => $cc,];
            }
            return $H;
        }
        ...
        found_rows($R, $Z)
        {
            ...
            $Z = where_to_query($Z);
            $lb = 'MongoDB\Driver\Command';
            $tb = new$lb(['count' => $R['Name'], 'query' => $Z]);
            $Ug = $h->_link->executeCommand($h->_db_name, $tb);
            $si = $Ug->toArray();
            return $si[0]->n;
        }
        ...
        sql_query_where_parser($Ag)
        {
            ...
            foreach ($pj
                     as $nj) $Z[] = trim($nj);
            if (count($qj) == 1) $qj = []; elseif (count($qj) > 1) $Z = [];
            return
                where_to_query($Z, $qj);
        }

        function
        where_to_query($lj = [], $mj = [])
        {
            global $b;
            $Pb = [];
            foreach (['and' => $lj, 'or' => $mj] as $T => $Z) {
                if (is_array($Z)) {
                    foreach ($Z
                             as $Lc) {
                        ...
                        if ($T == 'and') $Pb['$and'][] = [$ob => [$yf => $X]]; elseif ($T == 'or') $Pb['$or'][] = [$ob => [$yf => $X]];
                    }
                }
            }
            return $Pb;
        ...
        $_f = ["=", "!=", ">", "<", ">=", "<=", "regex", "(f)=", "(f)!=", "(f)>", "(f)<", "(f)>=", "(f)<=", "(date)=", "(date)!=", "(date)>", "(date)<", "(date)>=", "(date)<=",];
    }
    function
    table($u)
    {
    ...
    alter_indexes($Q, $c)
    {
        ...
        foreach ($c
                 as $X) {
            list($T, $B, $N) = $X;
            if ($N == "DROP") $H = $h->_db->command(["deleteIndexes" => $Q, "index" => $B]); else {
                $f = [];
                foreach ($N
                         as $e) {
                ...
                $H = $h->_db->selectCollection($Q)->ensureIndex($f, ["unique" => ($T == "UNIQUE"), "name" => $B,]);
            }
            if ($H['errmsg']) {
                $h->error = $H['errmsg'];
                return
...
if (isset($_GET["elastic"])) {
    ...
    if (function_exists('json_decode') && ini_bool('allow_url_fopen')) {
        ...
        Min_DB
        {
            ...
            function
            rootQuery($dg, $Bb = [], $We = 'GET')
            {
                @ini_set('track_errors', 1);
                $Xc = @file_get_contents("$this->_url/" . ltrim($dg, '/'), false, stream_context_create(['http' => ['method' => $We, 'content' => $Bb === null ? $Bb : json_encode($Bb), 'header' => 'Content-Type: application/json', 'ignore_errors' => 1,]]));
                if (!$Xc) {
        ...
        select($Q, $K, $Z, $rd, $Ef = [], $z = 1, $D = 0, $rg = false)
        {
            ...
            foreach ($Z
                ...
                if ($ob == "_id") $Pb["query"]["ids"]["values"][] = $X; elseif ($ob . $X != "") {
                    $fi = ["term" => [($ob != "" ? $ob : "_all") => $X]];
                    if ($yf == "=") $Pb["query"]["filtered"]["filter"]["and"][] = $fi; else$Pb["query"]["filtered"]["query"]["bool"]["must"][] = $fi;
                }
            }
            if ($Pb["query"] && !$Pb["query"]["filtered"]["query"] && !$Pb["query"]["ids"]) $Pb["query"]["filtered"]["query"] = ["match_all" => []];
            $Ih = microtime(true);
    ...
    {
        global $h;
        $hh = $h->query("_search", ["size" => 0, "aggregations" => ["count_by_type" => ["terms" => ["field" => "_type"]]]], "POST");
        $H = [];
        if ($hh) {
            $S = $hh["aggregations"]["count_by_type"]["buckets"];
            foreach ($S
                     as $Q) {
                $H[$Q["key"]] = ["Name" => $Q["key"], "Engine" => "table", "Rows" => $Q["doc_count"],];
    ...
    fields($Q)
    {
        ...
        if ($G) {
            ...
            if ($Ee) {
                foreach ($Ee
                         as $B => $o) {
                    $H[$B] = ["field" => $B, "full_type" => $o["type"], "type" => $o["type"], "privileges" => ["insert" => 1, "select" => 1, "update" => 1],];
                    if ($o["properties"]) {
    ...
    {
        global $h;
        return $h->rootQuery(urlencode(implode(',', $k)), [], 'DELETE');
    }
    ...
    drop_tables($S)
    {
        ...
        $H = true;
        foreach ($S
                 as $Q) $H = $H && $h->query(urlencode($Q), [], 'DELETE');
        return $H;
    }
...
if (isset($_GET["clickhouse"])) {
    ...
    Min_DB
    {
        ...
        {
            @ini_set('track_errors', 1);
            $Xc = @file_get_contents("$this->_url/?database=$l", false, stream_context_create(['http' => ['method' => 'POST', 'content' => $this->isQuerySelectLike($F) ? "$F FORMAT JSONCompact" : $F, 'header' => 'Content-type: application/x-www-form-urlencoded', 'ignore_errors' => 1,]]));
            if ($Xc === false) {
                $this->error = $php_errormsg;
                return $Xc;
            }
    ...
    unconvert_field($o, $H)
    {
        if (in_array($o['type'], ["Int8", "Int16", "Int32", "Int64", "UInt8", "UInt16", "UInt32", "UInt64", "Float32", "Float64"])) return "to$o[type]($H)";
        return $H;
    }
    ...
    fields($Q)
    {
        ...
        foreach ($G
            ...
            $H[trim($I['name'])] = ["field" => trim($I['name']), "full_type" => $T, "type" => $T, "default" => trim($I['default_expression']), "null" => $kf, "auto_increment" => '0', "privileges" => ["insert" => 1, "select" => 1, "update" => 0],];
        }
        return $H;
    }
...
if (!defined("DRIVER")) {
    ...
    if (extension_loaded("mysqli")) {
            ...
            Min_PDO
        {
            ...
            {
                global $b;
                $Cf = [PDO::MYSQL_ATTR_LOCAL_INFILE => false];
                $Hh = $b->connectSsl();
                if ($Hh) {
    ...
    foreign_keys($Q)
    {
        ...
        if ($Ib) {
            ...
            foreach ($Ie
                ...
                $H[idf_unescape($A[1])] = ["db" => idf_unescape($A[4] != "" ? $A[3] : $A[4]), "table" => idf_unescape($A[4] != "" ? $A[4] : $A[3]), "source" => array_map('idf_unescape', $Ah[0]), "target" => array_map('idf_unescape', $ci[0]), "on_delete" => ($A[6] ? $A[6] : "RESTRICT"), "on_update" => ($A[7] ? $A[7] : "RESTRICT"),];
            }
        }
        return $H;
    }
    ...
    {
        $H = [];
        foreach (get_rows("SHOW TRIGGERS LIKE " . q(addcslashes($Q, "%_\\"))) as $I) $H[$I["Trigger"]] = [$I["Timing"], $I["Event"]];
        return $H;
    }
    ...
    routine($B, $T)
    {
        ...
        foreach ($Ie
                 as $Sf) $p[] = ["field" => str_replace("``", "`", $Sf[2]) . $Sf[3], "type" => strtolower($Sf[5]), "length" => preg_replace_callback("~$_c~s", 'normalize_enum', $Sf[6]), "unsigned" => strtolower(preg_replace('~\s+~', ' ', trim("$Sf[8] $Sf[7]"))), "null" => 1, "full_type" => $Sf[4], "inout" => strtoupper($Sf[1]), "collation" => strtolower($Sf[9]),];
        if ($T != "FUNCTION") return
            ["fields" => $p, "definition" => $A[11]];
        return
            ["fields" => $p, "returns" => ["type" => $A[12], "length" => $A[13], "unsigned" => $A[15], "collation" => $A[16]], "definition" => $A[17], "language" => "SQL",];
    }
    ...
    $Ni = ["unsigned", "zerofill", "unsigned zerofill"];
    $_f = ["=", "<", ">", "<=", ">=", "!=", "LIKE", "LIKE %%", "REGEXP", "IN", "FIND_IN_SET", "IS NULL", "NOT LIKE", "NOT REGEXP", "NOT IN", "IS NOT NULL", "SQL"];
    $od = ["char_length", "date", "from_unixtime", "lower", "round", "floor", "ceil", "sec_to_time", "time_to_sec", "upper"];
    $ud = ["avg", "count", "count distinct", "group_concat", "max", "min", "sum"];
    $qc = [["char" => "md5/sha1/password/encrypt/uuid", "binary" => "md5/sha1", "date|time" => "now",], [number_type() => "+/-", "date" => "+ interval/- interval", "time" => "addtime/subtime", "char|text" => "concat",]];
}
...
Adminer
{
    ...
    selectColumnsPrint($K, $f)
    {
        ...
        foreach ($K
            ...
            echo "<div>" . ($od || $ud ? "<select name='columns[$s][fun]'>" . optionlist([-1 => ""] + array_filter([lang(55) => $od, lang(56) => $ud]), $X["fun"]) . "</select>" . on_help("getTarget(event).value && getTarget(event).value.replace(/ |\$/, '(') + ')'", 1) . script("qsl('select').onchange = function () { helpClose();" . ($y !== "" ? "" : " qsl('select, input', this.parentNode).onchange();") . " };", "") . "($e)" : $e) . "</div>\n";
            $s++;
        }
        echo "</div></fieldset>\n";
    }
                ...
                foreach ($rh
                    ...
                    foreach ($Xi
                        ...
                        if ($E !== null) {
                            ...
                            foreach (($Ub ? array_keys($Ub) : [""]) as $l) $Pf .= "<li><a href='" . h(auth_url($cj, $M, $V, $l)) . "'>($ic[$cj]) " . h($V . ($M != "" ? "@" . $this->serverName($M) : "") . ($l != "" ? " - $l" : "")) . "</a>\n";
                        }
                    }
                }
            }
...
';
                if ($S) {
                    ...
                    foreach (["bac", "bra", "sqlite_quo", "mssql_bra"] as $X) echo "jushLinks.$X = jushLinks.$x;\n";
                }
                $qh = $h->server_info;
...
';
        ...
        echo "<span title='" . lang(76) . "'>" . lang(77) . "</span>: " . ($k ? "<select name='db'>" . optionlist(["" => ""] + $k, DB) . "</select>$Sb" : "<input name='db' value='" . h(DB) . "' autocapitalize='off'>\n"), "<input type='submit' value='" . lang(20) . "'" . ($k ? " class='hidden'" : "") . ">\n";
        if ($Ye != "db" && DB != "" && $h->select_db(DB)) {
            if (support("scheme")) {
                echo "<br>" . lang(78) . ": <select name='ns'>" . optionlist(["" => ""] + $b->schemas(), $_GET["ns"]) . "</select>$Sb";
    ...
    tablesPrint($S)
    {
        ...
        foreach ($S
            ...
            $B = $this->tableName($O);
            if ($B != "") {
                echo '<li><a href="' . h(ME) . 'select=' . urlencode($Q) . '"' . bold($_GET["select"] == $Q || $_GET["edit"] == $Q, "select") . ">" . lang(79) . "</a> ", (support("table") || support("indexes") ? '<a href="' . h(ME) . 'table=' . urlencode($Q) . '"' . bold(in_array($Q, [$_GET["table"], $_GET["create"], $_GET["indexes"], $_GET["foreign"], $_GET["trigger"]]), (is_view($O) ? "view" : "structure")) . " title='" . lang(43) . "'>$B</a>" : "<span>$B</span>") . "\n";
            }
        }
        echo "</ul>\n";
    }
...
auth_error($n)
{
    ...
    echo "<form action='' method='post'>\n", "<div>";
    if (hidden_fields($_POST, ["auth"])) echo "<p class='message'>" . lang(90) . "\n";
    echo "</div>\n";
    $b->loginForm();
    echo "</form>\n";
    page_footer("auth");
    ...
    for ($s = 0; (!$z || $s < $z) && ($I = $G->fetch_row()); $s++) {
        if (!$s) {
            ...
            for ($ge = 0; $ge < count($I); $ge++) {
                ...
                $U[$ge] = $o->type;
                echo "<th" . ($Gf != "" || $o->name != $Ff ? " title='" . h(($Gf != "" ? "$Gf." : "") . $Ff) . "'" : "") . ">" . h($B) . ($Hf ? doc_link(['sql' => "explain-output.html#explain_" . strtolower($B), 'mariadb' => "explain/#the-columns-in-explain-select",]) : "");
            }
            echo "</thead>\n";
        }
        echo "<tr" . odd() . ">";
...
edit_type($y, $o, $qb, $id = [], $Pc = [])
{
    ...
    echo ' aria-labelledby="label-length"><td class="options">', "<select name='" . h($y) . "[collation]'" . (preg_match('~(char|text|enum|set)$~', $T) ? "" : " class='hidden'") . '><option value="">(' . lang(101) . ')' . optionlist($qb, $o["collation"]) . '</select>', ($Ni ? "<select name='" . h($y) . "[unsigned]'" . (!$T || preg_match(number_type(), $T) ? "" : " class='hidden'") . '><option>' . optionlist($Ni, $o["unsigned"]) . '</select>' : ''), (isset($o['on_update']) ? "<select name='" . h($y) . "[on_update]'" . (preg_match('~timestamp|datetime~', $T) ? "" : " class='hidden'") . '>' . optionlist(["" => "(" . lang(102) . ")", "CURRENT_TIMESTAMP"], (preg_match('~^CURRENT_TIMESTAMP~i', $o["on_update"]) ? "CURRENT_TIMESTAMP" : $o["on_update"])) . '</select>' : ''), ($id ? "<select name='" . h($y) . "[on_delete]'" . (preg_match("~`~", $T) ? "" : " class='hidden'") . "><option value=''>(" . lang(103) . ")" . optionlist(explode("|", $vf), $o["on_delete"]) . "</select> " : " ");
}
...
{
    return
        [idf_escape(trim($o["field"])), process_type($Fi), ($o["null"] ? " NULL" : " NOT NULL"), default_value($o), (preg_match('~timestamp|datetime~', $o["type"]) && $o["on_update"] ? " ON UPDATE $o[on_update]" : ""), (support("comment") && $o["comment"] != "" ? " COMMENT " . q($o["comment"]) : ""), ($o["auto_increment"] ? auto_increment() : null),];
}
...
grant($pd, $ug, $f, $uf)
{
    if (!$ug) return
        true;
    if ($ug == ["ALL PRIVILEGES", "GRANT OPTION"]) return ($pd == "GRANT" ? queries("$pd ALL PRIVILEGES$uf WITH GRANT OPTION") : queries("$pd ALL PRIVILEGES$uf") && queries("$pd GRANT OPTION$uf"));
    return
...
doc_link($eg, $hi = "<sup>?</sup>")
{
    ...
    $Si = ['sql' => "https://dev.mysql.com/doc/refman/$dj/en/", 'sqlite' => "https://www.sqlite.org/", 'pgsql' => "https://www.postgresql.org/docs/$dj/", 'mssql' => "https://msdn.microsoft.com/library/", 'oracle' => "https://www.oracle.com/pls/topic/lookup?ctx=db" . preg_replace('~^.* (\d+)\.(\d+)\.\d+\.\d+\.\d+.*~s', '\1\2', $qh) . "&id=",];
...
connect_error()
{
    ...
    if (DB != "") {
        ...
        echo "<p class='links'>\n";
        foreach (['database' => lang(115), 'privileges' => lang(72), 'processlist' => lang(116), 'variables' => lang(117), 'status' => lang(118),] as $y => $X) {
    ...
    foreach (table_status('', true) as $Q => $R) {
        ...
        foreach ($b->foreignKeys($Q) as $X) {
            if (!$X["db"]) {
                ...
                $fh[$Q]["references"][$X["table"]][(string)$ue] = [$X["source"], $X["target"]];
...
';
    ...
    if ($_POST && !$n) {
        $Fb = "";
        foreach (["output", "format", "db_style", "routines", "events", "table_style", "auto_increment", "triggers", "data_style"] as $y) $Fb .= "&$y=" . urlencode($_POST[$y]);
        ...
        foreach ((array)$k
            ...
            if ($h->select_db($l)) {
                ...
                if ($de) {
                    ...
                    $Of = "";
                    if ($_POST["routines"]) {
                        foreach (["FUNCTION", "PROCEDURE"] as $Zg) {
...
';
    ...
    $Yh = ['', 'DROP+CREATE', 'CREATE'];
    $Qb = ['', 'TRUNCATE+INSERT', 'INSERT'];
    if ($x == "sql") $Qb[] = 'INSERT+UPDATE';
    parse_str($_COOKIE["adminer_export"], $I);
    if (!$I) $I = ["output" => "text", "format" => "sql", "db_style" => (DB != "" ? "" : "CREATE"), "table_style" => "DROP+CREATE", "data_style" => "INSERT"];
    if (!isset($I["events"])) {
...
';
    ...
    if ($_POST["save"]) $I = (array)$_POST["fields"]; elseif ($Z) {
        ...
        if (!support("table")) $K = ["*"];
        if ($K) {
            $G = $m->select($a, $K, [$Z], $K, [], (isset($_GET["select"]) ? 2 : 1));
            if (!$G) $n = error(); else {
                $I = $G->fetch_assoc();
                if (!$I) $I = false;
            }
    ...
    $a = $_GET["create"];
    $Xf = [];
    foreach (['HASH', 'LINEAR HASH', 'KEY', 'LINEAR KEY', 'RANGE', 'LIST'] as $y) $Xf[$y] = $y;
    ...
    if ($_POST) set_adminer_settings(["comments" => $_POST["comments"], "defaults" => $_POST["defaults"]]);
    if ($_POST && !process_fields($I["fields"]) && !$n) {
        if ($_POST["drop"]) queries_redirect(substr(ME, 0, -1), lang(169), drop_tables([$a])); else {
            $p = [];
            $Ea = [];
            $Ti = false;
            $gd = [];
            ...
            foreach ($I["fields"] as $y => $o) {
                ...
                if ($o["field"] != "") {
                    ...
                    if ($q !== null) $gd[idf_escape($o["field"])] = ($a != "" && $x != "sqlite" ? "ADD" : " ") . format_foreign_key(['table' => $id[$o["type"]], 'source' => [$o["field"]], 'target' => [$Fi["field"]], 'on_delete' => $o["on_delete"],]);
    ...
    if (!$_POST) {
        $I = ["Engine" => $_COOKIE["adminer_engine"], "fields" => [["field" => "", "type" => (isset($U["int"]) ? "int" : (isset($U["integer"]) ? "integer" : "")), "on_update" => ""]], "partition_names" => [""],];
        if ($a != "") {
            $I = $R;
            $I["name"] = $a;
            $I["fields"] = [];
...
';
        ...
        echo($zc ? "<select name='Engine'>" . optionlist(["" => "(" . lang(173) . ")"] + $zc, $I["Engine"]) . "</select>" . on_help("getTarget(event).value", 1) . script("qsl('select').onchange = helpClose;") : ""), ' ', ($qb && !preg_match("~sqlite|mssql~", $x) ? html_select("Collation", ["" => "(" . lang(101) . ")"] + $qb, $I["Collation"]) : ""), ' <input type="submit" value="', lang(14), '">
';
    }
    echo '
';
...
';
    ...
    if (support("partitioning")) {
        ...
        echo '<p>
', "<select name='partition_by'>" . optionlist(["" => ""] + $Xf, $I["partition_by"]) . "</select>" . on_help("getTarget(event).value.replace(/./, 'PARTITION BY \$&')", 1) . script("qsl('select').onchange = partitionByChange;"), '(<input name="partition" value="', h($I["partition"]), '">)
...
';
    ...
    if ($_POST["add"]) {
        ...
        $v = end($I["indexes"]);
        if ($v["type"] || array_filter($v["columns"], 'strlen')) $I["indexes"][] = ["columns" => [1 => ""]];
    }
    if (!$I) {
        foreach ($w
                 as $y => $v) {
...
';
    ...
    foreach ($I["indexes"] as $v) {
        if (!$_POST["drop_col"] || $ge != key($_POST["drop_col"])) {
            echo "<tr><td>" . html_select("indexes[$ge][type]", [-1 => ""] + $Nd, $v["type"], ($ge == count($I["indexes"]) ? "indexesAddRow.call(this);" : 1), "label-type"), "<td>";
            ksort($v["columns"]);
            $s = 1;
            foreach ($v["columns"] as $y => $e) {
...
';
    ...
    page_header(DB != "" ? lang(68) : lang(115), $n, [], h(DB));
    $qb = collations();
    $B = DB;
...
<form action="" method="post">
<p>
', ($_POST["add_x"] || strpos($B, "\n") ? '<textarea id="name" name="name" rows="10" cols="40">' . h($B) . '</textarea><br>' : '<input name="name" id="name" value="' . h($B) . '" data-maxlength="64" autocapitalize="off">') . "\n" . ($qb ? html_select("collation", ["" => "(" . lang(101) . ")"] + $qb, $I["collation"]) . doc_link(['sql' => "charset-charsets.html", 'mariadb' => "supported-character-sets-and-collations/", 'mssql' => "ms187963.aspx",]) : ""), script("focus(qs('#name'));"), '<input type="submit" value="', lang(14), '">
';
...
';
    ...
    if ($_POST && !$n && !$_POST["add"] && !$_POST["change"] && !$_POST["change-js"]) {
        ...
        if ($x == "sqlite") queries_redirect($Be, $Qe, recreate_table($a, $a, [], [], [" $B" => ($_POST["drop"] ? "" : " " . format_foreign_key($I))])); else {
    ...
    if ($_POST) {
        ksort($I["source"]);
        if ($_POST["add"]) $I["source"][] = ""; elseif ($_POST["change"] || $_POST["change-js"]) $I["target"] = [];
    } elseif ($B != "") {
        $id = foreign_keys($a);
        $I = $id[$B];
        $I["source"][] = "";
...
';
    ...
    foreach ($I["source"] as $y => $X) {
        echo "<tr>", "<td>" . html_select("source[" . (+$y) . "]", [-1 => ""] + $Ah, $X, ($ge == count($I["source"]) - 1 ? "foreignAddRow.call(this);" : 1), "label-source"), "<td>" . html_select("target[" . (+$y) . "]", $ci, $I["target"][$y], 1, "label-target");
        $ge++;
    }
    echo '</table>
<p>
', lang(103), ': ', html_select("on_delete", [-1 => ""] + explode("|", $vf), $I["on_delete"]), ' ', lang(102), ': ', html_select("on_update", [-1 => ""] + explode("|", $vf), $I["on_update"]), doc_link(['sql' => "innodb-foreign-key-constraints.html", 'mariadb' => "foreign-keys/", 'pgsql' => "sql-createtable.html#SQL-CREATETABLE-REFERENCES", 'mssql' => "ms174979.aspx", 'oracle' => "https://docs.oracle.com/cd/B19306_01/server.102/b14200/clauses002.htm#sthref2903",]), '<p>
<input type="submit" value="', lang(14), '">
...
';
    ...
    page_header(($a != "" ? lang(44) : lang(204)), $n, ["table" => $a], h($a));
    echo '
<form action="" method="post">
...
';
    ...
    $Kh = ["ENABLED" => "ENABLE", "DISABLED" => "DISABLE", "SLAVESIDE_DISABLED" => "DISABLE ON SLAVE"];
    $I = $_POST;
    if ($_POST && !$n) {
...
';
    ...
    if ($_POST && !process_fields($I["fields"]) && !$n) {
        ...
        drop_create("DROP $Zg " . routine_id($da, $If), create_routine($Zg, $I), "DROP $Zg " . routine_id($I["name"], $I), create_routine($Zg, ["name" => $ei] + $I), "DROP $Zg " . routine_id($ei, $I), substr(ME, 0, -1), lang(214), lang(215), lang(216), $da, $I["name"]);
    }
...
';
    ...
    if (isset($_GET["function"])) {
        echo "<tr><td>" . lang(221);
        edit_type("returns", $I["returns"], $qb, [], ($x == "pgsql" ? ["void", "trigger"] : []));
    }
    echo '</table>
', script("editFields();"), '</div>
<p>';
    ...
    if ($_POST) {
        if (!$n && in_array($_POST["Timing"], $Di["Timing"]) && in_array($_POST["Event"], $Di["Event"]) && in_array($_POST["Type"], $Di["Type"])) {
            ...
            if ($_POST["drop"]) query_redirect($jc, $Be, lang(231)); else {
                ...
                if ($B != "") queries(create_trigger($uf, $I + ["Type" => reset($Di["Type"])]));
            }
        }
        $I = $_POST;
    }
    page_header(($B != "" ? lang(234) . ": " . h($B) : lang(235)), $n, ["table" => $a]);
    echo '
<form action="" method="post" id="form">
<table cellspacing="0" class="layout">
...
';
    ...
    $ug["Columns"] = [];
    foreach (["Select", "Insert", "Update", "References"] as $X) $ug["Columns"][$X] = $ug["Tables"][$X];
    ...
    page_header((isset($_GET["host"]) ? lang(36) . ": " . h("$ha@$_GET[host]") : lang(146)), $n, ["privileges" => ['', lang(72)]]);
    if ($_POST) {
        $I = $_POST;
        $qd = $df;
    } else {
        $I = $_GET + ["host" => $h->result("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', -1)")];
        $I["pass"] = $sf;
        if ($sf != "") $I["hashed"] = true;
        $qd[(DB == "" || $qd ? "" : idf_escape(addcslashes(DB, "%_\\"))) . ".*"] = [];
    }
    echo '<form action="" method="post">
<table cellspacing="0" class="layout">
...
';
    ...
    echo "</thead>\n";
    foreach (["" => "", "Server Admin" => lang(35), "Databases" => lang(38), "Tables" => lang(131), "Columns" => lang(49), "Procedures" => lang(242),] as $Db => $bc) {
...
', script("mixin(qsl('table'), {onclick: tableClick, ondblclick: partialArg(tableClick, true)});");
    ...
    foreach (process_list() as $s => $I) {
        if (!$s) {
            ...
            foreach ($I
                     as $y => $X) echo "<th>$y" . doc_link(['sql' => "show-processlist.html#processlist_" . strtolower($y), 'pgsql' => "monitoring-stats.html#PG-STAT-ACTIVITY-VIEW", 'oracle' => "REFRN30223",]);
            echo "</thead>\n";
        }
...
';
    ...
    header("Content-Type: text/javascript; charset=utf-8");
    if ($_GET["script"] == "db") {
        $Rh = ["Data_length" => 0, "Index_length" => 0, "Data_free" => 0];
        foreach (table_status() as $B => $R) {
            json_row("Comment-$B", h($R["Comment"]));
            if (!is_view($R)) {
                foreach (["Engine", "Collation"] as $y) json_row("$y-$B", h($R[$y]));
                foreach ($Rh + ["Auto_increment" => 0, "Rows" => 0] as $y => $X) {
                    if ($R[$y] != "") {
                        $X = format_number($R[$y]);
            ...
            if (!$Zh) echo "<p class='message'>" . lang(9) . "\n"; else {
                ...
                foreach ($Zh
                    ...
                    if ($ej) {
                        ...
                        foreach (["Engine" => [], "Collation" => [], "Data_length" => ["create", lang(45)], "Index_length" => ["indexes", lang(133)], "Data_free" => ["edit", lang(46)], "Auto_increment" => ["auto_increment=1&create", lang(45)], "Rows" => ["select", lang(42)],] as $y => $_) {
                            $t = " id='$y-" . h($B) . "'";
                ...
                foreach (["Data_length", "Index_length", "Data_free"] as $y) echo "<td align='right' id='sum-$y'>";
                echo "</table>\n", "</div>\n";
